...
import pm.models.Entities.Ghost;
import pm.models.Entities.PacMan;
import pm.tools.BreadthFirstPaths;
import pm.tools.Graph;
...
public class PacManGame implements Game {
    ...
    public PacManGame(PlayerListener playerListener) {
    ...
    public void checkForPause() {
        if (playerListener.hasPressedPause()) {
            ifPause = !ifPause;
            playerListener.resetPause(); // 将输入的P调成false
        }
    }
    ...
    private void checkForSkip() {
        if (playerListener.hasPressedSkip()) {
            if (currentLevel < LEVELS - 1) {
                playerListener.resetSkip();
                moveToNextLevel();
            } else {
                playerLives = 0;
            }
